...
from aplication.db import get_db
...
def register():
    try:
        ...
        if request.method == 'POST':
            ...
            jls_extract_var = hex
            salt = jls_extract_var(random.getrandbits(128))
            number = hex(random.getrandbits(512))
            db.execute(
                "INSERT INTO activationlink (challenge, state, username, password, salt, email) VALUES (?, ?, ?, ?, ?, ?)",
                (number, utils.U_UNCONFIRMED, username, generate_password_hash(password + salt), salt, email,)
            )
            db.commit()
